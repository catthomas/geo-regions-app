 ...
 * on a map with their geographic neighbors. It implements
 * recursive methods.
 * @version 1.0
 */
public class RecursiveGNMapView extends JFrame{
	/**
	 * Unique ID in order for this class to be written to files
	 * directly as an object.
	 */
	private static final long serialVersionUID = -7452893312240122159L;
	 ...
	 */
	public RecursiveGNMapView(Mappable<? extends Region> region, int breadth,
		...
		//ArrayList to hold visited regions in recursive call
		ArrayList<Mappable<? extends Region>> visitedRegions = new ArrayList();
		//Generate the geographic neighborhood
		ArrayList<Mappable<? extends Region>> neighbors = generateRecursiveGeoNeighbors(region,
				breadth, length, visitedRegions);
		//Add original region to neighborhood
		...
		for (Mappable<? extends Region> neighbor: neighbors) {
				...
				//Map the border for each region
				mapBorder(neighbor, breadth, length, mapImage, neighborPoint);
		} // end for loop
